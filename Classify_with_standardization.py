...
test_sets = [0] * 10
for t in train_test:
    fnames = glob.glob("studentsData/good/*_%s*.p"%(t))
    for fname in fnames:
...
def SaveDataToPickle(var, fname):
    pickle_out = open("mainData/"+fname,"wb")
...
if False: # using KNN_backup
    ...
    for i in range(0, testX.shape[0], step_size):
        ...
        compare_result = np.equal(predictions,testY[i:i+step_size])
        great = np.sum(compare_result)
        ...
        if great!=step_size:
            ...
            print(testX[i].shape)
            for index, err in enumerate(compare_result):
                if err==False:
                    show.show_hand(testX[i+index].reshape(5,4,6), "errorReport/err_pred_%d_testY_%d_%d.png"%(predictions[index], testY[i+index], i+index))
            #exit()
...
# After standardization, the accuracy is almost 100%.
pickle.dump(knn, open('mainData/classifier_with_standardization.p','wb'))
